 ...
 */
function execute(
  { name, args }: Extract<MainThreadMessage, { type: 'execute' }>['action'],
): WorkerThreadMessage {
  if (name === '__constructor__') {
    const [debug, size] = args as ConstructorParameters<typeof Webview>;
  ...
  if (name === 'return') {
    const [seq, status, result] = args as Parameters<Webview['return']>;
  ...
  if (name === 'size') {
    ...
    return { type: 'result', return: { name: 'size', value: [webview?.size] } };
  }
  if (name === 'title') {
  ...
  if (name === 'unsafeHandle') {
    ...
    return {
      type: 'result',
      return: { name: 'unsafeHandle', value: [webview?.unsafeHandle] },
    };
  }
